...
const docTypes: DocType[] = ["Recommendation Letter", "Attestation Letter", "Letter of Good Standing", "Other"];
...
const DocumentRequests = () => {
  ...
  useEffect(() => {
    ...
    apiFetch<any>("/students/me").then((data) => {
      if (data && data.id) {
        ...
        toast({ title: "Student record not found", description: "Unable to find your student record. Contact administrator.", variant: "destructive" });
      }
    }).catch((err) => {
      toast({ title: "Error loading student data", description: err instanceof ApiError ? err.message : "Cannot connect to server", variant: "destructive" });
    });
  }, [user?.id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!studentId) {
      toast({ title: "Cannot submit request", description: "Student ID not found. Please refresh the page.", variant: "destructive" });
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast({ title: "Failed to submit request", description: err instanceof ApiError ? err.message : "An error occurred. Please try again.", variant: "destructive" });
    } finally {
      setSubmitting(false);
    }
  };
