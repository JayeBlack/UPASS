...
CREATE TYPE user_role AS ENUM ('Student', 'Supervisor', 'Accountant', 'Admin', 'Dean', 'ViceDean', 'Registrar', 'ExamsOfficer', 'AdminAssistant', 'AccountingAssistant');
...
ALTER TABLE clearance_steps ADD CONSTRAINT clearance_steps_student_dept_unique UNIQUE (student_id, department);
...
ALTER TABLE clearance_steps ADD CONSTRAINT clearance_steps_supervisor_user_id_fkey FOREIGN KEY (supervisor_user_id) REFERENCES users(id) ON DELETE SET NULL;
...
-- Table: student_resource_reads
CREATE TABLE student_resource_reads (id integer NOT NULL DEFAULT nextval('student_resource_reads_id_seq'::regclass), student_id integer, item_type character varying(50) NOT NULL, item_id integer NOT NULL, created_at timestamp without time zone DEFAULT now());
ALTER TABLE student_resource_reads ADD CONSTRAINT student_resource_reads_pkey PRIMARY KEY (id);
ALTER TABLE student_resource_reads ADD CONSTRAINT student_resource_reads_student_id_item_type_item_id_key UNIQUE (student_id, item_type, item_id);
ALTER TABLE student_resource_reads ADD CONSTRAINT student_resource_reads_student_id_fkey FOREIGN KEY (student_id) REFERENCES students(id) ON DELETE CASCADE;
