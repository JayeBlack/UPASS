...
exports.uploadForStudents = async (req, res) => {
  ...
  try {
    ...
    let fileUrl;
    if (useCloudinary) {
      const result = await uploadToCloudinary(req.file.buffer, req.file.originalname, "upass/documents");
    ...
    if (request_id) {
      ...
      if (requestInfo.rows.length > 0) {
        await createNotification(
          requestInfo.rows[0].user_id, "document", "Document Ready",
      ...
      for (const studentId of studentIdArray) {
        ...
        if (studentQuery.rows.length > 0) {
          await createNotification(
            studentQuery.rows[0].user_id, "document", "New Document Available",
